...
export default function AssetsPage() {
  ...
  const chainBalances = [
    { name: 'Arc Network', balance: '1,240.50', icon: '🟣', color: 'bg-violet-600', status: 'Main' },
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-fade-in pb-16">
      ...
      <div className="flex flex-col gap-1.5">
        <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight flex items-center gap-3">
          Global Liquidity
          <span className="text-[10px] font-black bg-white/5 border border-white/10 px-2 py-0.5 rounded uppercase tracking-widest text-gray-400">Beta</span>
        </h1>
      ...
      {!isConnected && (
        <div className="p-6 rounded-3xl bg-violet-600/10 border border-violet-500/20 flex flex-col sm:flex-row items-center justify-between gap-4 animate-fade-in shadow-[0_0_40px_rgba(124,58,237,0.1)]">
          <div className="flex items-center gap-4 text-center sm:text-left">
            ...
            <div>
              <h3 className="text-sm font-black text-white uppercase tracking-tight">Identity Required</h3>
      ...
      <div className="glass-panel p-8 rounded-[3rem] relative overflow-hidden bg-gradient-to-br from-violet-600/20 via-[#0A0A0F] to-[#0A0A0F] border border-white/5 shadow-2xl">
        ...
        <div className="relative z-10 space-y-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">Total Consolidated Value</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-end gap-2">
            ...
            <span className="text-xl font-black text-violet-400 mb-2 uppercase tracking-widest">USDC</span>
          </div>

          <div className="flex flex-wrap gap-4 pt-4">
            <div className="flex items-center gap-2 px-4 py-2 rounded-2xl bg-white/5 border border-white/5">
              <Layers className="w-4 h-4 text-blue-400" />
              <span className="text-xs font-bold text-white">4 Active Networks</span>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 rounded-2xl bg-white/5 border border-white/5">
              <ShieldCheck className="w-4 h-4 text-emerald-400" />
              <span className="text-xs font-bold text-white">Verified On-chain</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {chainBalances.map((chain, idx) => (
          <div 
            key={chain.name} 
            className="glass-panel p-6 rounded-[2rem] bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all group relative overflow-hidden"
          >
            ...
            <div className="flex items-center justify-between relative z-10">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center text-xl shadow-lg border border-white/5 group-hover:scale-110 transition-transform">
                  {chain.icon}
                </div>
                <div>
                  <h4 className="text-sm font-black text-white">{chain.name}</h4>
                  <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{chain.status}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-lg font-black text-white">${isConnected ? chain.balance : '0.00'}</p>
                <p className="text-[9px] font-black text-violet-400 uppercase tracking-widest">USDC</p>
              </div>
            </div>
          </div>
        ))}
      ...
      {/* ── FOOTER INFO ── */}
      <div className="flex flex-col sm:flex-row items-center justify-between p-6 rounded-3xl bg-white/[0.01] border border-white/5 gap-4">
        ...
        <button className="text-[10px] font-black text-violet-400 hover:text-violet-300 uppercase tracking-widest flex items-center gap-2 transition-all group">
          Refresh Indices
