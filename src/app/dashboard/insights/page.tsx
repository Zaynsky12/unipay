    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-16">
      ...
      <div className="flex flex-col gap-1.5">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight">AI Sales Assistant</h1>
        </div>
      ...
      {!isConnected && (
        <div className="p-6 rounded-3xl bg-violet-600/10 border border-violet-500/20 flex flex-col sm:flex-row items-center justify-between gap-4 animate-fade-in shadow-[0_0_40px_rgba(124,58,237,0.1)]">
          <div className="flex items-center gap-4 text-center sm:text-left">
            ...
            <div>
              <h3 className="text-sm font-black text-white uppercase tracking-tight">AI Agent Offline</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="glass-panel p-6 rounded-[2rem] bg-white/[0.02] border border-white/5 space-y-4">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-white leading-tight">Sales expected to rise by 12% next week.</p>
          </div>
        </div>
        <div className="glass-panel p-6 rounded-[2rem] bg-white/[0.02] border border-white/5 space-y-4">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-white leading-tight">3 new repeat buyers detected today.</p>
          </div>
        </div>
        <div className="glass-panel p-6 rounded-[2rem] bg-white/[0.02] border border-white/5 space-y-4">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-white leading-tight">Buyers are most active at 7:00 PM UTC.</p>
          </div>
        </div>
      </div>

      {/* ── CHAT INTERFACE ── */}
      <div className="glass-panel rounded-[2.5rem] border border-white/5 bg-[#0A0A0F] overflow-hidden flex flex-col h-[400px] shadow-2xl relative">
        ...
        <div className="flex-1 p-8 flex flex-col items-center justify-center text-center space-y-4 relative z-10">
          <div className="w-16 h-16 rounded-3xl bg-violet-600/10 border border-violet-500/20 flex items-center justify-center text-violet-400 animate-bounce">
            <MessageSquare className="w-8 h-8" />
          </div>
          <div>
            <h3 className="text-lg font-black text-white">Ask your Sales Assistant</h3>
        ...
        <div className="p-6 bg-white/[0.02] border-t border-white/5 relative z-10">
          <div className="flex flex-wrap gap-2 mb-4 justify-center">
            {aiSuggestions.map((text, i) => (
              <button 
                ...
                className="text-[9px] font-black uppercase tracking-tighter bg-white/5 hover:bg-white/10 border border-white/5 rounded-full px-3 py-1.5 text-gray-400 hover:text-violet-300 transition-all"
              >
                {text}
              </button>
            ))}
          ...
          <div className="relative group">
            <input 
              ...
              className="w-full bg-black/60 border border-white/10 rounded-2xl py-4 pl-6 pr-14 text-sm text-white placeholder:text-gray-600 outline-none focus:border-violet-500/50 transition-all shadow-inner"
            ...
            <button 
              className={`absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-xl flex items-center justify-center transition-all ${
                query ? 'bg-violet-600 text-white shadow-lg shadow-violet-600/30' : 'bg-white/5 text-gray-600'
              } disabled:opacity-30`}
